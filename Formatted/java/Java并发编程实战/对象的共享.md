...
#### 可见性&emsp;  
&emsp;  
​&emsp;&emsp;当一个线程修改了对象（变量）的状态（值）后，其他线程能够看到状态发生的变化。&emsp;  
&emsp;  
#### 失效数据&emsp;  
&emsp;  
​&emsp;&emsp;若对象或变量在不同的线程中共享，且对象的状态或变量的值被某一个线程改变，但其他线程在读取时，并不会读取到最新的值，而是数据被修改之前的值，此时读取到的数据就是**失效数据**。&emsp;  
&emsp;  
​&emsp;&emsp;&emsp;  
&emsp;  
#### Synchronized与volatile&emsp;  
&emsp;  
​&emsp;&emsp;Synchronized不仅可以对变量实现同步，也可以实现内存的可见性。&emsp;  
&emsp;  
​&emsp;&emsp;volatile可以只可以实现内存的可见性，不能对变量实现同步，相对Synchronized来说更加轻量级。&emsp;  
&emsp;  
​&emsp;&emsp;volatile虽然更加轻量级，但不能滥用。应仅当在volatile能简化代码实现时或对同步策略的验证时使用，即在以下情况使用：&emsp;  
&emsp;  
> 1. 确保对象自身状态的可见性&emsp;  
> 2. 确保变量多引用对象的状态的可见性&emsp;  
> 3. 标识一些重要的程序生命周期事件的发生&emsp;  
&emsp;  
​&emsp;&emsp;volatile可以使用的条件：&emsp;  
&emsp;  
> 1. 对变量的写入操作不依赖变量的当前值，或能确保只有单个线程更新变量&emsp;  
> 2. 该变量不会与其他状态变量一起纳入不变性条件中&emsp;  
> 3. 在访问变量时不需要加锁&emsp;  
&emsp;  
## 发布与溢出&emsp;  
&emsp;  
​&emsp;&emsp;发布：发布一个对象指的是，使对象能够在当前作用域之外的代码中使用。&emsp;  
&emsp;  
​&emsp;&emsp;逸出：当某个不应该发布的对象被发布时，就叫作逸出。&emsp;  
&emsp;  
​&emsp;&emsp;对象发布的方法：&emsp;  
&emsp;  
> 1. 将对象的引用保存到一个公有的静态变量中&emsp;  
> 2. 将对象的引用传递到非对象所属类中的方法&emsp;  
> 3. 在构造函数中创建一个新的线程&emsp;  
&emsp;  
​&emsp;&emsp;在第三种情况中，this引用会逸出到新的线程中，在运行新的线程时，此对象可能还未构造完成，就可能会出错。&emsp;  
&emsp;  
若必须要i在构造函数中创建新的线程，可以先不调用.start()方法，对象创建完成后再调用。&emsp;  
&emsp;  
## 线程封闭&emsp;  
&emsp;  
​&emsp;&emsp;如果仅在单线程内访问数据，就不需要使用同步，这种技术被称为线程封闭，这是实现线程安全性的最简单方式之一。&emsp;  
&emsp;  
#### Ad-hoc线程封闭&emsp;  
&emsp;  
​&emsp;&emsp;Ad hoc是拉丁文常用短语中的一个短语。这个短语通常用来形容一些特殊的、不能用于其它方面的的，为一个特定的问题、任务而专门设定的解决方案。所以此种线程封闭完全由程序的实现者来手动根据具体情况来实现。&emsp;  
&emsp;  
#### 栈封闭&emsp;  
&emsp;  
...
&emsp;  
​&emsp;&emsp;每当需要一对相关数据以原子方式执行某个操作时，就可以考虑创建一个不可变的类来包含这些数据，这样就不会有其他的线程修改这些数据。如果要更新这些变量，那么可以创建一个新的容器对象。在使用不可变对象的同时，可以使用 volatile来修饰不可变对象的引用变量，确保可见性，使所有线程都可以获得最新的对象引用。&emsp;  
&emsp;  
## 安全发布&emsp;  
&emsp;  
#### 不可变对象&emsp;  
