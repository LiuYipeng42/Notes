...
​		可以利用堆（PriorityQueue）和快速排序来解决。

​		在快速排序的算法中，每次（getIndex方法）找到的都是 L 位置上的数的正确排序位置，这个位置左边的数小于 L 位置上的数，右边的数大于 L 位置上的数。所以要找到 k 个最小的数，就要找到一个正确排序位置为 k 的数，这个数的左边就是 k 个最小的数。
